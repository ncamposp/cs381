...
cmd (Pen m') (m, p) = ((m', p), Nothing)
cmd (Move x y) (n, (i, j)) = case n of 
                                Up -> ((Up, (expr x, expr y)),  Nothing)
                                Down ->  ((Down,(expr x, expr y)) , Just ((i,j),(expr x,expr y)))
...
block (c:cs) s = case cmd c s of 
                (s', Just l) -> (fst (block cs s') , l : (snd(block cs s')))
                (s', Nothing) -> (fst (block cs s') , snd(block cs s'))
...
optimize :: Prog -> Prog
optimize [] = []
optimize (c :cs) = case c of
                    Pen _ -> c: optimize cs 
                    Move x y -> Move (Lit (expr x)) (Lit (expr y)) : optimize cs 
